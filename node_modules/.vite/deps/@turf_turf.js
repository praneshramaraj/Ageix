...
function removeBbox(geojson) {
	...
	} else if (geojson.type === "FeatureCollection") geojson.features.forEach(removeBbox);
	else if (geojson.type === "GeometryCollection") geojson.geometries.forEach(removeBbox);
}
...
function propEach(geojson, callback) {
	...
	switch (geojson.type) {
		case "FeatureCollection":
			...
			break;
		case "Feature": callback(geojson.properties, 0);
	}
}
	...
	flattenEach(geojson, function(feature2, featureIndex, multiFeatureIndex) {
		...
		switch (type) {
			case "LineString":
				...
				break;
			case "Polygon": for (var geometryIndex = 0; geometryIndex < coords.length; geometryIndex++) if (callback(lineString(coords[geometryIndex], feature2.properties), featureIndex, multiFeatureIndex, geometryIndex) === false) return false;
		}
	});
}
...
function polygonclip(points, bbox) {
	var result, edge = 1, prev, prevInside, i, p, inside;
	for (; edge <= 8; edge *= 2) {
		result = [];
...
var splitter$1 = 134217729;
var resulterrbound$1 = 3.000000000000001 * epsilon$3;
...
var ccwerrboundA$1 = (3 + 16 * epsilon$3) * epsilon$3;
var ccwerrboundB$1 = (2 + 12 * epsilon$3) * epsilon$3;
var ccwerrboundC$1 = (9 + 64 * epsilon$3) * epsilon$3 * epsilon$3;
var B$1 = vec$1(4);
var C1$1 = vec$1(8);
var C2$1 = vec$1(12);
var D$1 = vec$1(16);
...
function orient2dadapt$1(ax, ay, bx, by, cx, cy, detsum) {
	...
	_i = s0 - t0;
	bvirt = s0 - _i;
	u$5[0] = s0 - (_i + bvirt) + (bvirt - t0);
	_j = s1 + _i;
	bvirt = _j - s1;
	_0 = s1 - (_j - bvirt) + (_i - bvirt);
	_i = _0 - t1;
	bvirt = _0 - _i;
	u$5[1] = _0 - (_i + bvirt) + (bvirt - t1);
	u3 = _j + _i;
	bvirt = u3 - _j;
	u$5[2] = _j - (u3 - bvirt) + (_i - bvirt);
	u$5[3] = u3;
	const C1len = sum$1(4, B$1, 4, u$5, C1$1);
	s1 = acx * bcytail;
	c = splitter$1 * acx;
	ahi = c - (c - acx);
	alo = acx - ahi;
	...
	u$5[2] = _j - (u3 - bvirt) + (_i - bvirt);
	u$5[3] = u3;
	const C2len = sum$1(C1len, C1$1, 4, u$5, C2$1);
	s1 = acxtail * bcytail;
	c = splitter$1 * acxtail;
	ahi = c - (c - acxtail);
	alo = acxtail - ahi;
	...
	u$5[2] = _j - (u3 - bvirt) + (_i - bvirt);
	u$5[3] = u3;
	return D$1[sum$1(C2len, C2$1, 4, u$5, D$1) - 1];
}
...
//#endregion
//#region node_modules/@turf/line-intersect/node_modules/tinyqueue/index.js
...
function testSegmentIntersect(seg1, seg2) {
	...
	if (seg1.leftSweepEvent.ringId === seg2.leftSweepEvent.ringId) {
		if (seg1.rightSweepEvent.isSamePoint(seg2.leftSweepEvent) || seg1.rightSweepEvent.isSamePoint(seg2.rightSweepEvent) || seg1.leftSweepEvent.isSamePoint(seg2.leftSweepEvent) || seg1.leftSweepEvent.isSamePoint(seg2.rightSweepEvent)) return false;
	} else {
...
function nearestPointOnSegment(posA, posB, posC) {
	...
	if (segmentAxis[0] === 0 && segmentAxis[1] === 0 && segmentAxis[2] === 0) {
		if (dot$1(A, B) > 0) return [[...posB], true];
...
function booleanContains(feature1, feature2) {
	...
	switch (type1) {
		case "Point": switch (type2) {
			case "Point": return compareCoords$2(coords1, coords2);
		...
		case "LineString": switch (type2) {
			...
			case "LineString": return isLineOnLine$1(geom1, geom2);
		...
		case "Polygon": switch (type2) {
			...
			case "LineString": return isLineInPoly$1(geom1, geom2);
			case "Polygon": return isPolyInPoly$1(geom1, geom2);
		...
		case "MultiPolygon": switch (type2) {
			case "Point": return isPointInMultiPolygon(geom1, geom2);
			case "MultiPoint": return isMultiPointInMultiPolygon(geom1, geom2);
			case "LineString": return isLineInMultiPolygon(geom1, geom2);
			case "MultiLineString": return isMultiLineStringInMultiPolygon(geom1, geom2);
			case "Polygon": return isPolygonInMultiPolygon(geom1, geom2);
			case "MultiPolygon": return isMultiPolygonInMultiPolygon(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
...
function isPolygonInMultiPolygon(multiPolygon, polygon) {
	...
	return multiPolygon.coordinates.some((coords) => isPolyInPoly$1({
		type: "Polygon",
		coordinates: coords
	}, polygon, polygonBbox));
}
...
function isMultiPointInMultiPolygon(multiPolygon, multiPoint) {
	...
	for (const coord of multiPoint.coordinates) {
		if (!booleanPointInPolygon(coord, multiPolygon)) return false;
		if (!oneInside) oneInside = booleanPointInPolygon(coord, multiPolygon, { ignoreBoundary: true });
	}
	return oneInside;
}
function isLineInMultiPolygon(multiPolygon, lineString2) {
	return multiPolygon.coordinates.some((coords) => isLineInPoly$1({
		type: "Polygon",
		coordinates: coords
	}, lineString2));
}
function isMultiLineStringInMultiPolygon(multiPolygon, multiLineString) {
	...
	for (const lineCoords of multiLineString.coordinates) {
		...
		for (const polyCoords of multiPolygon.coordinates) {
			...
			if (status === "boundary") lineStatus = status;
		}
		if (lineStatus === "outside") return false;
		if (lineStatus === "interior") oneInterior = true;
	}
	return oneInterior;
}
function isMultiPolygonInMultiPolygon(multiPolygon1, multiPolygon2) {
	for (const poly2Coords of multiPolygon2.coordinates) {
		...
		const poly2Bbox = bbox(poly2);
		if (!multiPolygon1.coordinates.some((poly1Coords) => isPolyInPoly$1({
			type: "Polygon",
			coordinates: poly1Coords
		}, poly2, poly2Bbox))) return false;
	}
	return true;
}
function isMultiPolyInPoly(polygon, multiPolygon) {
	return multiPolygon.coordinates.every((coords) => isPolyInPoly$1(polygon, {
		type: "Polygon",
		coordinates: coords
	}));
}
function isPointInMultiPoint(multiPoint, pt) {
	let i;
	let output = false;
	for (i = 0; i < multiPoint.coordinates.length; i++) if (compareCoords$2(multiPoint.coordinates[i], pt.coordinates)) {
		output = true;
		break;
	}
	return output;
...
function isMultiPointInMultiPoint(multiPoint1, multiPoint2) {
	for (const coord2 of multiPoint2.coordinates) {
		...
		for (const coord1 of multiPoint1.coordinates) if (compareCoords$2(coord2, coord1)) {
			matchFound = true;
			break;
		}
...
function isMultiPointOnLine(lineString2, multiPoint) {
	...
	for (const coord of multiPoint.coordinates) {
		...
		if (!haveFoundInteriorPoint && booleanPointOnLine(coord, lineString2, { ignoreEndVertices: true })) haveFoundInteriorPoint = true;
	}
...
function isMultiPointInPoly(polygon, multiPoint) {
	let oneInside = false;
	for (const coord of multiPoint.coordinates) {
		if (!booleanPointInPolygon(coord, polygon)) return false;
		if (!oneInside) oneInside = booleanPointInPolygon(coord, polygon, { ignoreBoundary: true });
	}
	return oneInside;
}
function isLineOnLine$1(lineString1, lineString2) {
	...
	for (let i = 0; i < coordinates.length; i++) {
		...
		if (!booleanPointOnLine(coords, lineString1)) return false;
		if (!haveFoundInteriorPoint) {
			if (booleanPointOnLine(coords, lineString1, { ignoreEndVertices: true })) haveFoundInteriorPoint = true;
			else if (i > 0) {
				if (booleanPointOnLine([(coordinates[i - 1][0] + coords[0]) / 2, (coordinates[i - 1][1] + coords[1]) / 2], lineString1, { ignoreEndVertices: true })) haveFoundInteriorPoint = true;
			}
		}
	}
	return haveFoundInteriorPoint;
...
function lineInPolyStatus(polygon, linestring) {
	if (!doBBoxOverlap(bbox(polygon), bbox(linestring))) return "outside";
	for (const coord of linestring.coordinates) if (!booleanPointInPolygon(coord, polygon)) return "outside";
	...
	for (const lineSegment of lineSegments.features) {
		...
		if (!booleanPointInPolygon(midpoint, polygon)) return "outside";
		if (!hasInteriorSegment && booleanPointInPolygon(midpoint, polygon, { ignoreBoundary: true })) hasInteriorSegment = true;
	}
	return hasInteriorSegment ? "interior" : "boundary";
}
function isPolyInPoly$1(feature1, feature2, feature2Bbox) {
	if (feature1.type === "Feature" && feature1.geometry === null) return false;
	if (feature2.type === "Feature" && feature2.geometry === null) return false;
	if (!doBBoxOverlap(bbox(feature1), feature2Bbox != null ? feature2Bbox : bbox(feature2))) return false;
	const poly1 = getGeom(feature1);
	const coords = getGeom(feature2).coordinates;
	for (const ring of coords) {
		for (const coord of ring) if (!booleanPointInPolygon(coord, feature1)) return false;
		const segments = splitLineIntoSegmentsOnPolygon({
			type: "LineString",
			coordinates: ring
		}, poly1);
		for (const segment of segments.features) {
			const midpoint = getMidpoint(segment.geometry.coordinates[0], segment.geometry.coordinates[1]);
			if (!booleanPointInPolygon(midpoint, feature1) && !isPointOnPolygonBoundary(midpoint, poly1)) return false;
		}
	}
	return true;
}
...
function isPointOnPolygonBoundary(point, polygon) {
	return polygon.coordinates.some((ring) => {
		for (let i = 0; i < ring.length - 1; i++) if (pointToSegmentDistance(point, ring[i], ring[i + 1]) <= BOUNDARY_DISTANCE_TOLERANCE) return true;
		return false;
	});
}
...
function doMultiPointAndLineStringCross(multiPoint, lineString) {
	...
	for (var i = 0; i < pointLength && (!foundIntPoint || !foundExtPoint); i++) {
		...
		for (var i2 = 0; i2 < lineString.coordinates.length - 1; i2++) {
			...
			if (isPointOnLineSegment$1(lineString.coordinates[i2], lineString.coordinates[i2 + 1], multiPoint.coordinates[i], incEndVertices)) {
				pointOnLine = true;
				break;
			}
		}
		if (pointOnLine) foundIntPoint = true;
...
function doLineStringsCross(lineString1, lineString2) {
	...
	for (const intersectPoint of doLinesIntersect.features) if (!booleanEqual(intersectPoint, point(lineString1.coordinates[0])) && !booleanEqual(intersectPoint, point(lineString1.coordinates[lineString1.coordinates.length - 1])) && !booleanEqual(intersectPoint, point(lineString2.coordinates[0])) && !booleanEqual(intersectPoint, point(lineString2.coordinates[lineString2.coordinates.length - 1]))) return true;
	return false;
}
...
function disjoint(geom1, geom2, ignoreSelfIntersections) {
	switch (geom1.type) {
		case "Point":
			switch (geom2.type) {
				case "Point": return !compareCoords$1(geom1.coordinates, geom2.coordinates);
		...
		case "LineString":
			switch (geom2.type) {
				...
				case "LineString": return !isLineOnLine(geom1, geom2, ignoreSelfIntersections);
				case "Polygon": return !isLineInPoly(geom2, geom1, ignoreSelfIntersections);
			}
			break;
		case "Polygon": switch (geom2.type) {
			case "Point": return !booleanPointInPolygon(geom2, geom1);
			case "LineString": return !isLineInPoly(geom1, geom2, ignoreSelfIntersections);
			case "Polygon": return !isPolyInPoly(geom2, geom1, ignoreSelfIntersections);
		}
	}
	return false;
}
...
function isLineOnLine(lineString1, lineString2, ignoreSelfIntersections) {
	...
	for (const coords of lineString1.coordinates) if (isPointOnLine(lineString2, {
		type: "Point",
		coordinates: coords
	})) return true;
	for (const coords of lineString2.coordinates) if (isPointOnLine(lineString1, {
		type: "Point",
		coordinates: coords
	})) return true;
	return false;
...
function isPointOnLineSegment(lineSegmentStart, lineSegmentEnd, pt) {
	...
	if (Math.abs(dxl) >= Math.abs(dyl)) {
		if (dxl > 0) return lineSegmentStart[0] <= pt[0] && pt[0] <= lineSegmentEnd[0];
	...
	segmentEach(line2, function(segment) {
		...
		featureEach(tree.search(search), function(match) {
			if (doesOverlaps === false) {
				...
				} else if (tolerance === 0 ? booleanPointOnLine(coordsMatch[0], segment) && booleanPointOnLine(coordsMatch[1], segment) : nearestPointOnLine(segment, coordsMatch[0]).properties.pointDistance <= tolerance && nearestPointOnLine(segment, coordsMatch[1]).properties.pointDistance <= tolerance) {
					if (overlapSegment) {
...
function booleanOverlap(feature1, feature2) {
	...
	switch (type1) {
		...
		case "MultiLineString":
			...
			break;
		case "Polygon":
		case "MultiPolygon": segmentEach(feature1, (segment1) => {
			segmentEach(feature2, (segment2) => {
...
function booleanValid(feature) {
	...
	switch (type) {
		...
		case "GeometryCollection":
			...
			return Array.isArray(geom.geometries) && geom.geometries.length > 0 && geom.geometries.every((geometry) => GEOMETRY_COLLECTION_ENTRY_ALLOWED_TYPES.has(geometry.type) && booleanValid(geometry));
...
//#endregion
//#region node_modules/@turf/jsts/dist/jsts.min.js
var require_jsts_min = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(t, e) {
		...
		}, A.MAX_VALUE = Number.MAX_VALUE, A.POSITIVE_INFINITY = Number.POSITIVE_INFINITY, A.NEGATIVE_INFINITY = Number.NEGATIVE_INFINITY, "function" == typeof Float64Array && "function" == typeof Int32Array ? (R = 2146435072, w = /* @__PURE__ */ new Float64Array(1), O = new Int32Array(w.buffer), A.doubleToLongBits = function(t) {
			w[0] = t;
			var e = 0 | O[0], n = 0 | O[1];
...
function longitude(point) {
	...
	else return sign$1(point[0]) * ((abs(point[0]) + pi) % tau - pi);
}
...
function circle_default(radius) {
	var cr = cos(radius), delta = 6 * radians, smallRadius = cr > 0, notHemisphere = abs(cr) > epsilon$1;
...
function projectionMutator(projectAt) {
	...
	projection.stream = function(stream) {
		return cache && cacheStream === stream ? cache : cache = transformRadians(transformRotate$1(rotate)(preclip(projectResample(postclip(cacheStream = stream)))));
	};
...
var { BufferOp, GeoJSONReader, GeoJSONWriter } = (/* @__PURE__ */ __toESM(require_jsts_min())).default;
...
function convex(geojson, options = {}) {
	...
	if (convexHull.length > 3) return polygon([convexHull], options.properties);
	return null;
}
//#endregion
...
function clustersDbscan(points, maxDistance, options = {}) {
	...
	const regionQuery = (index) => {
		...
		return tree.search(bbox).filter((neighbor) => {
			...
			return distance(point, neighborPoint, { units: options.units }) <= maxDistance;
		});
	};
...
var buffer$1 = /* @__PURE__ */ new ArrayBuffer(16);
var floats = new Float64Array(buffer$1);
var uints = new Uint32Array(buffer$1);
...
function cut_default(topology) {
	var junctions = join_default(topology), coordinates = topology.coordinates, lines = topology.lines, rings = topology.rings, next, i = 0, n = lines.length;
	for (; i < n; ++i) {
...
function distanceWeight(fc, options) {
	...
	for (let i = 0; i < features.length; i++) for (let j = 0; j < features.length; j++) {
		...
		if (dis === 0) continue;
		if (binary) {
			if (dis <= threshold) weights[i][j] = 1;
			else weights[i][j] = 0;
...
function createMask(mask2) {
	...
	if (mask2) {
		if (mask2.type === "Feature") coordinates = mask2.geometry.coordinates;
...
var Graph$1 = class _Graph {
	...
	*/
	addEdge(from, to) {
		var _a;
		if ((_a = this.edges.get(from)) == null ? void 0 : _a.has(to)) return;
	...
	*/
	_computeNextCWEdges(node) {
		if (node == null) this._forEachNode((node2) => this._computeNextCWEdges(node2));
	...
	*/
	removeNode(node) {
		...
		(_a = this.nodes.get(node.coordinates[0])) == null || _a.delete(node.coordinates[1]);
	}
	/**
	...
	*/
	removeEdge(edge) {
		var _a;
		(_a = this.edges.get(edge.from)) == null || _a.delete(edge.to);
		edge.deleteEdge();
	}
	_forEachNode(fn) {
		for (const latMap of this.nodes.values()) for (const node of latMap.values()) fn(node);
	}
	_forEachEdge(fn) {
		for (const toMap of this.edges.values()) for (const edge of toMap.values()) fn(edge);
	}
};
...
function removeBeach(beach) {
	...
	disappearing.push(rArc);
	detachCircle(rArc);
	var nArcs = disappearing.length, iArc = 1;
	for (; iArc < nArcs; ++iArc) {
